...
void search_maze() {
    ...
    if (right != 0 && (count != junction_nodes[index] || junction_visited[index] < 1)) {
        if (left != 0 || front != 0) {
            index++;
    ...
    else if (front != 0 && (count != junction_nodes[index] || junction_visited[index] < 2)) {
        if (left != 0) {
            index++;
            junction_nodes[index] = count;
        }
    ...
    else if (left != 0 && (count != junction_nodes[index] || junction_visited[index] < 3)) {
        Serial.println(F("Turn left now"));
        turnLeft90(mpuState, bearingState);
    ...
    {
        if ((junction_visited[index] == 2 && left == 0) || junction_visited[index] == 3)
        {
